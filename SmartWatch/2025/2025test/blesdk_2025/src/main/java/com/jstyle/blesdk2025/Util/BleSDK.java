  ...
  private static  boolean readhrv=false;
   ...
   protected static  boolean Obtain_The_data_of_manual_blood_oxygen_test;
    ...
    public static byte[] SetWomenHealth(WomenHealth womenHealth) {
        ...
        byte[] value = new byte[16];
        value[0] = DeviceConst.WomenHealth;
        value[1] = (byte)ResolveUtil.getBcd(String.valueOf(Integer.parseInt(time[1].trim()))); //月
        value[2] = (byte)ResolveUtil.getBcd(String.valueOf(Integer.parseInt(time[2].trim())));//日
        value[3] = (byte)ResolveUtil.getBcd(String.valueOf(womenHealth.getMenstrualPeriod_Lenth())); //经期长度，范围2~8天
        value[4] = (byte)ResolveUtil.getBcd(String.valueOf(womenHealth.getMenstrualPeriod_Period())); //周期长度，范围21~35天
        crcValue(value);
        return value;
    }
    ...
    public static byte[] SetPregnancyCycle(PregnancyCycle pregnancyCycle) {
        ...
        byte[] value = new byte[16];
        value[0] = DeviceConst.PregnancyCycle;
        value[3] = (byte) ResolveUtil.getBcd(String.valueOf(Integer.parseInt(time[0].trim())-2000));//年
        value[4] = (byte) ResolveUtil.getBcd(String.valueOf(Integer.parseInt(time[1].trim())));//月
        value[5] = (byte)ResolveUtil.getBcd(String.valueOf(Integer.parseInt(time[2].trim())));//日
        crcValue(value);
        return value;
    }
